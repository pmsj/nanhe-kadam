 <div class="bg-neutral text-neutral-content grid lg:grid-cols-2 items-center shadow hover:shadow-lg hover:shadow-primary p-5 rounded-xl">
     <div class="">
        @if($article->getFirstMediaUrl('article-images'))
        <x-card class="bg-neutral">
            <x-slot:figure>
          ...
          <p class="">{!! $article->truncatedBody() !!}</p>
           <x-slot:actions separator class="justify-between">
                <div class="space-x-2">
                    ...
                    <x-button label="Read more" class="btn-secondary btn-sm" link="{{ route('articles.show', $article->slug) }}" />
                </div>
            </x-slot:actions>
        </x-card>
     </div>
